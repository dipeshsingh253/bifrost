...
import { ArrowLeft, Copy, MailPlus, Shield, Trash2, UserRoundX, Users } from "lucide-react";
...
export default function AdminPage({ currentUser, invites: initialInvites, tenant: initialTenant, viewers: initialViewers }: AdminPageProps) {
  ...
  async function handleInviteSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const invite = await readPayload<ViewerInvite>(response);
      ...
      <Layout currentUser={currentUser}>
        <div className="mb-6 flex flex-col gap-4">
          <nav className="flex items-center gap-1.5 text-sm text-muted-foreground">
            <Link className="inline-flex items-center gap-1 hover:text-foreground transition-colors" href="/">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-foreground">Tenant Access</h1>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_1.1fr]">
          <section className="rounded-xl border border-border bg-card p-5">
            <div className="mb-4">
              <h2 className="text-lg font-semibold text-foreground">Invite Viewer</h2>
              <p className="mt-1 text-sm text-muted-foreground">
          ...
          <section className="rounded-xl border border-border bg-card p-5">
            ...
            <div className="grid gap-3">
              {invites.length === 0 ? (
                ...
                invites.map((invite) => (
                  <div key={invite.id} className="flex flex-col gap-3 rounded-lg border border-border bg-background/60 p-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
        ...
        <section className="mt-6 rounded-xl border border-border bg-card p-5">
          <div className="mb-4">
            <h2 className="text-lg font-semibold text-foreground">Viewer Accounts</h2>
            <p className="mt-1 text-sm text-muted-foreground">
          ...
          <div className="grid gap-3">
            {viewers.length === 0 ? (
              ...
              viewers.map((viewer) => (
                <div key={viewer.id} className="flex flex-col gap-4 rounded-lg border border-border bg-background/60 p-4 lg:flex-row lg:items-center lg:justify-between">
                  <div className="min-w-0">
