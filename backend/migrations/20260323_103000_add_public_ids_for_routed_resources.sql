...
ALTER TABLE system_onboardings
  ADD COLUMN public_id TEXT,
  ADD COLUMN server_public_id TEXT;

CREATE UNIQUE INDEX servers_public_id_key
  ON servers (public_id)
  WHERE public_id IS NOT NULL;

CREATE UNIQUE INDEX service_groups_public_id_key
  ON service_groups (public_id)
  WHERE public_id IS NOT NULL;

CREATE UNIQUE INDEX containers_public_id_key
  ON containers (public_id)
  WHERE public_id IS NOT NULL;

CREATE UNIQUE INDEX system_onboardings_public_id_key
  ON system_onboardings (public_id)
  WHERE public_id IS NOT NULL;

CREATE UNIQUE INDEX system_onboardings_server_public_id_key
