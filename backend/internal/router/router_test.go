...
func selfEnrollAgent(t *testing.T, router http.Handler, bootstrapKey, agentID, serverID string) string {
	...
	request := httptest.NewRequest(http.MethodPost, "/api/v1/agent/enroll", strings.NewReader(requestBody))
...
func TestBootstrapStatusReportsWhenSetupIsRequired(t *testing.T) {
	router := New(config.Config{}, store.NewMemoryStore(store.SeedData{}))
...
func TestInstallAgentScriptIsPublicAndContainsExpectedSetupSteps(t *testing.T) {
	...
	if !strings.Contains(body, `AGENT_BINARY_URL="${BIFROST_AGENT_BINARY_URL:-}"`) {
		t.Fatalf("expected install script to leave the binary download url empty when binary serving is disabled, got %q", body)
	}
...
func TestInstallAgentScriptDefaultsBinaryURLWhenBinaryServingIsEnabled(t *testing.T) {
	tempDir := t.TempDir()
	...
	if !strings.Contains(response.Body.String(), `AGENT_BINARY_URL="${BIFROST_AGENT_BINARY_URL:-http://example.com/api/v1/agent/install}"`) {
		t.Fatalf("expected install script to default the binary download url when binary serving is enabled, got %q", response.Body.String())
	}
}
...
func TestInstallAgentBinaryServesConfiguredBinary(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodGet, "/api/v1/agent/install", nil)
	request.Host = "example.com"
	response := httptest.NewRecorder()
	router.ServeHTTP(response, request)
...
func TestInstallAgentBinaryReturns503WhenNotConfigured(t *testing.T) {
	router := New(config.Config{}, store.NewMemoryStore(store.SeedData{}))
	...
	if response.Code != http.StatusServiceUnavailable {
		t.Fatalf("expected install binary status 503, got %d", response.Code)
	}

	requireErrorCode(t, response, "AGENT_BINARY_UNAVAILABLE")
}

func TestOversizedSnapshotBodyReturns413(t *testing.T) {
	router := New(config.Config{}, store.NewMemoryStore(seed.Data()))

	hugeMessage := strings.Repeat("x", 5<<20)
	requestBody := `{"agent_id":"` + seed.AgentIDDemo + `","server":{"id":"` + seed.ServerIDDevServer + `","name":"Dev Server","hostname":"devbox","status":"up","collected_at":"2026-03-23T03:00:00Z","services":[]},"logs":[{"server_id":"` + seed.ServerIDDevServer + `","service_id":"` + seed.ServiceIDServiceA + `","container_id":"` + seed.ContainerIDWeb1 + `","level":"info","message":"` + hugeMessage + `","timestamp":"2026-03-23T03:00:00Z"}]}`
	request := httptest.NewRequest(http.MethodPost, "/api/v1/agent/snapshot", bytes.NewBufferString(requestBody))
	request.Header.Set("Content-Type", "application/json")
	...
	requireErrorCode(t, response, "REQUEST_TOO_LARGE")
}

func TestSnapshotRateLimitReturns429AfterBurst(t *testing.T) {
	router := New(config.Config{}, store.NewMemoryStore(seed.Data()))

	requestBody := `{"agent_id":"` + seed.AgentIDDemo + `","server":{"id":"` + seed.ServerIDDevServer + `","name":"Dev Server","hostname":"devbox","status":"up","collected_at":"2026-03-23T03:00:00Z","services":[]}}`
	var last *httptest.ResponseRecorder
	for range 13 {
		request := httptest.NewRequest(http.MethodPost, "/api/v1/agent/snapshot", strings.NewReader(requestBody))
		request.Header.Set("Content-Type", "application/json")
		request.Header.Set("X-Agent-Key", "demo-agent-key")
		response := httptest.NewRecorder()
		router.ServeHTTP(response, request)
	...
	if last == nil || last.Code != http.StatusTooManyRequests {
		t.Fatalf("expected snapshot burst to be rate limited, got %d", last.Code)
	}
	requireErrorCode(t, last, "RATE_LIMITED")
}
...
func TestBootstrapLoginSessionAndLogoutFlow(t *testing.T) {
	...
	if cookies[0].Secure {
		t.Fatalf("expected direct http bootstrap cookie to be non-secure")
	}
...
func TestBootstrapCookieUsesSecureFlagBehindTrustedHTTPSProxy(t *testing.T) {
	router := New(config.Config{
		TrustedProxies: []string{"192.0.2.0/24"},
	}, store.NewMemoryStore(store.SeedData{}))

	bootstrapBody := `{"tenant_name":"Bifrost Local","name":"Admin User","email":"admin@example.com","password":"password123"}`
	request := httptest.NewRequest(http.MethodPost, "/api/v1/auth/bootstrap", strings.NewReader(bootstrapBody))
	request.Header.Set("Content-Type", "application/json")
	...
	if response.Code != http.StatusCreated {
		t.Fatalf("expected bootstrap status 201, got %d", response.Code)
	}
	...
	if !cookies[0].Secure {
		t.Fatalf("expected trusted https proxy bootstrap cookie to be secure")
	}
}

func TestUntrustedProxyHeaderDoesNotSetSecureCookie(t *testing.T) {
	router := New(config.Config{}, store.NewMemoryStore(store.SeedData{}))
	...
	if cookies[0].Secure {
		t.Fatalf("expected untrusted proxy header to be ignored for cookie security")
	}
}
...
func TestOversizedAuthBodyReturns413(t *testing.T) {
	router := New(config.Config{}, store.NewMemoryStore(seed.Data()))

	payload := `{"email":"owner@bifrost.local","password":"` + strings.Repeat("x", 70<<10) + `"}`
	request := httptest.NewRequest(http.MethodPost, "/api/v1/auth/login", bytes.NewBufferString(payload))
	request.Header.Set("Content-Type", "application/json")
	response := httptest.NewRecorder()
	router.ServeHTTP(response, request)
...
func TestLoginRateLimitReturns429AfterBurst(t *testing.T) {
	router := New(config.Config{}, store.NewMemoryStore(seed.Data()))

	loginBody := `{"email":"owner@bifrost.local","password":"bad-password"}`
	var last *httptest.ResponseRecorder
	for range 6 {
		request := httptest.NewRequest(http.MethodPost, "/api/v1/auth/login", strings.NewReader(loginBody))
		request.Header.Set("Content-Type", "application/json")
		response := httptest.NewRecorder()
		router.ServeHTTP(response, request)
		last = response
	...
	if last == nil || last.Code != http.StatusTooManyRequests {
		t.Fatalf("expected login burst to be rate limited, got %d", last.Code)
	}
	requireErrorCode(t, last, "RATE_LIMITED")
}
...
func TestAuthenticatedUserCanUpdateProfileName(t *testing.T) {
	...
	updateRequest := httptest.NewRequest(http.MethodPatch, "/api/v1/auth/me", strings.NewReader(updateBody))
...
func TestAuthenticatedUserCanChangePassword(t *testing.T) {
	...
	changeRequest := httptest.NewRequest(http.MethodPost, "/api/v1/auth/me/password", strings.NewReader(changeBody))
...
func TestAdminSummaryAllowsAdminAndBlocksViewer(t *testing.T) {
	router := New(config.Config{}, store.NewMemoryStore(seedWithViewer()))
...
func TestAdminCanCreateSystemOnboardingWithOpaqueAgentCredentials(t *testing.T) {
	...
	if payload.Data.InstallScriptURL != "https://bifrost.example.com/api/v1/agent/install.sh" {
	...
	if !strings.Contains(payload.Data.SystemdInstallCommand, "curl -fsSL 'https://bifrost.example.com/api/v1/agent/install.sh'") {
...
func TestSystemOnboardingTransitionsToConnectedAfterFirstIngest(t *testing.T) {
	...
	preEnrollRequest := httptest.NewRequest(http.MethodPost, "/api/v1/agent/snapshot", strings.NewReader(`{
	...
	ingestRequest := httptest.NewRequest(http.MethodPost, "/api/v1/agent/snapshot", strings.NewReader(ingestBody))
	...
	secondIngestRequest := httptest.NewRequest(http.MethodPost, "/api/v1/agent/snapshot", strings.NewReader(strings.Replace(ingestBody, collectedAt, "2026-03-23T03:00:00Z", 1)))
...
func TestPendingBootstrapTokenMustSelfEnrollBeforeSnapshotIngest(t *testing.T) {
	...
	preEnrollRequest := httptest.NewRequest(http.MethodPost, "/api/v1/agent/snapshot", strings.NewReader(ingestBody))
	preEnrollRequest.Header.Set("Content-Type", "application/json")
	preEnrollRequest.Header.Set("X-Agent-Key", createPayload.Data.APIKey)
	preEnrollResponse := httptest.NewRecorder()
	router.ServeHTTP(preEnrollResponse, preEnrollRequest)
	...
	postEnrollRequest := httptest.NewRequest(http.MethodPost, "/api/v1/agent/snapshot", strings.NewReader(ingestBody))
...
func TestAdminCanReissuePendingSystemOnboardingCredentials(t *testing.T) {
	...
	oldKeyRequest := httptest.NewRequest(http.MethodPost, "/api/v1/agent/snapshot", strings.NewReader(oldKeyIngest))
	...
	newKeyRequest := httptest.NewRequest(http.MethodPost, "/api/v1/agent/snapshot", strings.NewReader(oldKeyIngest))
...
func TestIngestSnapshotReplacesAndClearsDockerServiceState(t *testing.T) {
	...
	ingestSnapshot := func(body string) {
		request := httptest.NewRequest(http.MethodPost, "/api/v1/agent/snapshot", strings.NewReader(body))
		request.Header.Set("Content-Type", "application/json")
		request.Header.Set("X-Agent-Key", "demo-agent-key")
