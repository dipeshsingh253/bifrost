...
func TestCreateRejectsBlankName(t *testing.T) {
	service := NewService(NewRepository(store.NewMemoryStore(seed.Data()), shareddb.QueryTimeouts{}), "https://bifrost.example.com", "bifrost-agent:latest")
...
func TestCreateGetAndListRoundTripSystemOnboardingView(t *testing.T) {
	...
	if created.InstallScriptURL != "https://bifrost.example.com/api/v1/agent/install.sh" {
	...
	if !strings.Contains(created.SystemdInstallCommand, "curl -fsSL 'https://bifrost.example.com/api/v1/agent/install.sh'") {
	...
	detail, err := service.Get(context.Background(), seed.TenantIDDemo, created.ID)
	if err != nil {
	...
	list, err := service.List(context.Background(), seed.TenantIDDemo)
	if err != nil {
...
func TestReissueKeepsPendingOnboardingIdentityAndRotatesCredentials(t *testing.T) {
	...
	reissued, err := service.Reissue(context.Background(), seed.TenantIDDemo, created.ID)
	if err != nil {
...
func TestCancelRejectsConnectedOnboarding(t *testing.T) {
	...
	service := NewService(NewRepository(store, shareddb.QueryTimeouts{}), "https://bifrost.example.com", "bifrost-agent:latest")
	...
	err = service.Cancel(context.Background(), seed.TenantIDDemo, created.ID)
